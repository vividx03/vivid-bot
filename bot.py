...
# in_memory=True container restart par stale session lock hone se rokta hai
app = Client("vivid_pro_bot", api_id=API_ID, api_hash=API_HASH, bot_token=BOT_TOKEN, in_memory=True)
...
async def start_cmd(client, message):
    print(f"📩 /start command received from: {message.chat.id}", flush=True)
    text = """```
╔══════════════════════════════════╗
...
# ============================================================
# HEALTH SERVER (Isolated in a Daemon Thread)
# ============================================================
def run_health_server():
    """Ye server alag thread me chalega taaki Telegram loop 100% free rahe"""
...
# ============================================================
async def main():
    # 1. Health server ko alag thread me background me run karein
    health_thread = threading.Thread(target=run_health_server, daemon=True)
    health_thread.start()

    # 2. Telegram client start karein
    print("⏳ Starting Telegram client...", flush=True)
    await app.start()
    me = await app.get_me()
    print(f"🚀 Bot is running 24/7 as @{me.username}", flush=True)

    # 3. Live scheduler task start karein
    asyncio.create_task(scheduler_worker(app))
    print("⏰ Background scheduler worker started successfully!", flush=True)

    # 4. Pyrogram official idle loop (keeps connection open and processes updates)
    await idle()
    await app.stop()
